...
class Level():
    ...
    '''
    def __init__(self, h = HEIGHT, w = WIDTH, setting = CHAOTIC):
        random.seed()
        if h <= 0:
            h = HEIGHT
        if w <= 0:
            w = WIDTH
        self.level_list = self.initRawLevel(h, w)
        if setting == CHAOTIC:
            for i in xrange(int((h * w) / random.randrange(1, int((h * w) / 100)))):
                ...
                self.fillRect(y, x, dy, dx, TERRAIN_FLOOR)
        self.borderRect(0, 0, h, w, TERRAIN_WALL)
    ...
    '''
    def createAdjList(self, terrain_id):
        D = dict()
        for y in xrange(len(self.level_list)):
            for x in xrange(len(self.level_list[y])):
                L = []
                pos = 0
                D[Vertex(x, y)] = []
                L = self.getSample(y, x)
                for cell in L:
                    ...
                    if pos == BOTTOM_RIGHT and cell == terrain_id:
                        D[Vertex(x, y)].append(Vertex(x+1,y+1))
                    pos += 1
        D['HEIGHT'] = len(self.level_list)
        D['WIDTH']  = len(self.level_list[0]) # Remember, square 2d lists.
        D['TYPE'] = terrain_id
        return D
    ...
    '''
    def initRawLevel(self, h, w):
        raw_level = self.newMatrix(h)
    ...
    returned.
    '''
    def fillRect(self, y, x, h, w, terrain_id):
        if y < 0 or x < 0:
            return False
        try:
            self.level_list[y+h-1][x+w-1] # Remember, square 2d lists.
        except IndexError:
            return False
        for row in xrange(y,y+h):
            for col in xrange(x,x+w):
                self.level_list[row][col] = terrain_id
        return True
    ...
    returned.
    '''
    def borderRect(self, y, x, h, w, terrain_id):
        if y < 0 or x < 0:
            return False
        try:
            self.level_list[y+h-1][x+w-1] # Remember, square 2d lists.
            ...
            return False
        for col in xrange(x,x+w):
            self.level_list[y][col] = self.level_list[y+h-1][col] = terrain_id
        for row in xrange(y,y+h):
            self.level_list[row][x] = self.level_list[row][x+w-1] = terrain_id
        return True
    ...
    found.
    '''
    def testArea(self, y, x, h, w, terrain_id):
        offset_y = offset_x = 0
        if y < 0:
            offset_y = y
            y = 0
        ...
        for row in xrange(y,y+h+offset_y):
            for col in xrange(x,x+w+offset_x):
                try:
                    result = (terrain_id == self.level_list[row][col])
                except IndexError:
                    break
                else:
                    if result:
